...
class TextWindow(object):
    ...
    def read_log_record(self, client):
        ...
        if chunk == b"close":
            ...
            self.stext.winfo_toplevel().wm_attributes('-alpha', 0.0)
            return None
        elif chunk == b"show":
            self.stext.winfo_toplevel().wm_attributes('-alpha', 1.0)
            return None
        else:
    ...
    def __init__(self, log_format=None):
        ...
        self.sock.listen(1)
        self.stext = ScrolledText.ScrolledText(bg='white', height=20, width=100, state="disabled")
        self.stext.winfo_toplevel().title('Caster logging')
        self.stext.winfo_toplevel().wm_attributes('-topmost')
        ...
        print('Listening on %d' % logging.handlers.DEFAULT_TCP_LOGGING_PORT)
        self.stext.mainloop()
...
def run():
    ...
    TextWindow('%(asctime)-15s %(name)-10s %(levelname)-8s %(message)s')
